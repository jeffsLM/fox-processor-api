...
class ProcessQueueByPlusProviderController {
  async handle(_: Request, response: Response): Promise<Response> {
    ...
    const episodesToReprocess = await listEpisodeAnyTermController.handle({
      where: { url: ILike(`%google%`) },
    });

    // await Promise.all(
    //   queueToReprocess.map(async (animeQueue) => {
    ...
    return response.status(201).json(episodesToReprocess).send();
  }
}
